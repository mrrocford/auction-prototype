...
def viewdetails(request, lot_id):
    ...
    if request.method == "POST" and "createbid" in request.POST and request.POST["bidprice"] != '':
        ...
        oldprice = Lot.objects.values_list(
            'lot_price', flat=True).get(pk=lot_id)
        ...
        if lastbidprice <= oldprice:
            ...
            newbid = Bid.objects.create(
                bid_user=biduser, bid_price=lastbidprice, bid_lot_id=lot_id)
            newbid.save()
        ...
        print(
            f"comuser={commentuser}, comtext={commenttext}, comdat={commentdate}")
        newcomment = Comment.objects.create(
            comment_user=commentuser, comment_text=commenttext, comment_date=commentdate, comment_lot_id=lot_id)
        newcomment.save()
        return HttpResponseRedirect(reverse("viewdetails", args=(current_lot.id,)))
...
def createlot(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            lot = Lot.objects.create(lot_name=lot_name, lot_price=lot_price, lot_description=lot_description, lot_date=lot_date,
                                     lot_status=lot_status, lot_author=lot_author, lot_category=lot_category, lot_viewimage=lot_viewimage)
            lot.save()
...
def watchlist(request, user_id):
    ...
    if request.method == "POST" and "delwatchlist" in request.POST:
        ...
        watchlist = Watchlist.objects.create(
            watch_user=watchuser, watch_lot_id=watchlotid)
        watchlist.save()
        return render(request, "auctions/watchlist.html", {
            "lotimages": LotImage.objects.all(),
            "userinf": User.objects.all(),
        ...
        print(
            f"watchlistcat={Watchlist.objects.values_list('watch_lot_id__lot_category', flat=True).get(pk=1)}")
        return render(request, "auctions/watchlist.html", {
